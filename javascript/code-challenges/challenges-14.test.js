...
const screenForNames = (arr) => {
  let newArray = [];
  arr.map((screen)=>{
    if (screen.match(/^(Mr. |Mrs. |Ms. |Dr. )[a-zA-Z]/g)){
      newArray.push(screen);
    }
  });
  return newArray;
};
...
const toTitleCase = (arr) => arr.map((characterCase) => characterCase[0].toUpperCase() + characterCase.slice(1));
