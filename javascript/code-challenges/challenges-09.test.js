...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((arr, number) => Math.max(arr, number), 0);
...
const updateNumbers = (obj) => Object.keys(obj).map(phone => `${phone}: ${obj[phone]}`);
...
const getHouses = (arr) => {
  let houses = [];
  Object.values(arr).forEach(name => houses.push(name.house));
  return houses;
};
  ...
  arr.forEach(people =>{
    if (people.name === character){
      Object.keys(people).forEach((key, index) => {
        if (key ==='children'){ children = Object.values(people)[index].length;
        }
      });
    }
  });
