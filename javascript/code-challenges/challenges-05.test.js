...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map(full => full.firstName + ' ' + full.lastName);
...
------------------------------------------------------------------------------------------------ */
const addValues = (arr) => arr.reduce((acc, number) => acc + number,0);
...
const addPurchases = (arr) => arr.reduce((acc, number) => acc + number.purchasePrice,0);
...
const countNumberOfElements = (arr) => arr.reduce((acc, number) => acc + 1, 0);
...
const returnNames = (arr) => {
  let nameFinder = arr.reduce((acc, characters) => {acc.push(characters.name); return acc; 
  }, []);
  return nameFinder;
};
...
const reversedString = (str) =>{ return str.split('').reduce((acc,str)=>str + acc, '');};
